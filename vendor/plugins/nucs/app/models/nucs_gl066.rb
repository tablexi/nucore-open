class NucsGl066 < ActiveRecord::Base
  ...
  def starts_at
    return (self[:starts_at].nil? && !budget_period.nil?) ? Time.zone.parse("#{budget_period}0901") : self[:starts_at]
  end
  ...
  #
  # Returns true if the current time is in between +#starts_at+ and
  # +#expires_at+. false otherwise.
  def expired?
    return expires_at <= Time.zone.now || starts_at > Time.zone.now
  end
