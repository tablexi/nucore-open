...
describe Account do
  ...
  it "should return error if the product facility does not accept purchase orders" do
    po_account = FactoryGirl.create(:purchase_order_account, :account_users_attributes => [{:user => @user, :created_by => @user, :user_role => 'Owner'}])
  ...
  it "should return error if the product facility does not accept credit cards" do
    cc_account=FactoryGirl.create(:credit_card_account, :account_users_attributes => [{:user => @user, :created_by => @user, :user_role => 'Owner'}])
