...
module Reservations::DateSupport
  ...
  def duration_mins
    if @duration_mins
      ...
      duration_with_seconds_stripped(reserve_start_at, reserve_end_at) / 60
    else
      0
    end
  end
  ...
  def actual_duration_mins(base_time = Time.zone.now)
    if @actual_duration_mins
      ...
      [(duration_with_seconds_stripped(actual_start_at, actual_end_at) / 60), 1].max
    elsif actual_start_at
      [(duration_with_seconds_stripped(actual_start_at, base_time) / 60), 1].max
    else
      0
    end
  end
  ...
  def duration_with_seconds_stripped(start_time, end_time)
    end_time.change(sec: 0) - start_time.change(sec: 0)
  end
end
