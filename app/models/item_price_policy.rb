class ItemPricePolicy < PricePolicy
  ...
  named_scope :current,  lambda { |item|             { :conditions => [dateize('start_date', ' = ? AND item_id = ?'), current_date(item, true), item.id] } }
  named_scope :next,     lambda { |item|             { :conditions => [dateize('start_date', ' = ? AND item_id = ?'), next_date(item, true), item.id] } }
  ...
  def self.current_date(item, with_time=false)
    ipp = item.item_price_policies.find(:first, :conditions => [dateize('start_date', ' <= ? AND ') + dateize('expire_date', ' > ?'), Time.zone.now, Time.zone.now], :order => 'start_date DESC')
