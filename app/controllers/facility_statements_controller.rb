class FacilityStatementsController < ApplicationController
  ...
  def create
    ...
    if @statement_creator.order_detail_ids.blank?
      ...
      flash[:notice] = text(success_message, accounts: @statement_creator.formatted_account_list.html_safe).html_safe
    else
      flash[:error] = text("errors_html", errors: @statement_creator.formatted_errors.html_safe).html_safe
    end
