...
describe SettingsHelper do
  ...
  context 'fiscal year' do
    ...
    it 'should get the previous year for dates before the start' do
      expect(SettingsHelper::fiscal_year_beginning(DateTime.new(2010, 2, 5))).to eq(Time.zone.parse('2009-03-01').beginning_of_day)
    ...
    it 'should the the next year for dates after the start' do
      expect(SettingsHelper::fiscal_year_beginning(DateTime.new(2010, 5, 5))).to eq(Time.zone.parse('2010-03-01').beginning_of_day)
    end
    it 'should set the end of the fiscal year for dates before the start' do
      expect(SettingsHelper::fiscal_year_end(DateTime.new(2014, 2, 5))).to eq(Time.zone.parse('2014-02-28').end_of_day)
    end
    it 'should set the end of the fiscal year for dates after the start' do
      expect(SettingsHelper::fiscal_year_end(DateTime.new(2014, 5, 14))).to eq(Time.zone.parse('2015-02-28').end_of_day)
    end
  end
