...
RSpec.describe Reports::GeneralReportsController do
  ...
  context "report searching" do
    before :each do
      @complete_status = OrderStatus.complete_status
      ...
      @order_detail_ordered_yesterday_fulfilled_yesterday_reconciled_today.change_status!(OrderStatus.reconciled_status)
      ...
      @order_detail_ordered_yesterday_fulfilled_yesterday_reconciled_yesterday.change_status!(OrderStatus.reconciled_status)
    ...
    context "ordered_at search" do
      ...
      it "should search reconciled" do
        @params[:status_filter] = [OrderStatus.reconciled_status.id]
        do_request
  ...
  def setup_extra_params(params)
    params.merge!(status_filter: [OrderStatus.complete_status.id], date_range_field: "fulfilled_at")
  end
  ...
  def assert_report_params_init
    ...
    stati = if @params[:date_start].blank? && @params[:date_end].blank?
              [OrderStatus.complete_status, OrderStatus.reconciled_status]
