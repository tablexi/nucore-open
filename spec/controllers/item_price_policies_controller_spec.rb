...
describe ItemPricePoliciesController do
  ...
  before(:each) do
    ...
    @price_policy     = @item.item_price_policies.create(Factory.attributes_for(:item_price_policy).update(:price_group_id => @price_group.id))
    @price_policy.should be_valid
    @params={ :facility_id => @authable.url_name, :item_id => @item.url_name }
  end
  ...
  context "edit" do
    ...
    it 'should not allow edit of assigned effective price policy' do
      ...
      @order_detail = @order.order_details.create(Factory.attributes_for(:order_detail).update(:product_id => @item.id, :account_id => @account.id, :price_policy => @price_policy))
