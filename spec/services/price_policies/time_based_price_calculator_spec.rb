  ...
  context "when product has duration pricing mode" do
    ...
    describe "#calculate" do
      ...
      context "with duration rates" do
        context "one of them with minimum duration of 0 hours" do
          context "and rounded to hour time range" do
            let(:options) { { usage_rate: 120 } }
            let(:start_at) { Time.zone.local(2017, 4, 27, 12, 0) }
            let(:end_at) { start_at + 1.hour }

            it "uses duration rates rate" do
              create(:duration_rate, instrument: product, min_duration: 0, rate: 60)

              is_expected.to eq(cost: 60)
            end
          end
          ...
          context "and not rounded to hour time range" do
            ...
            it "uses duration rates rate" do
              ...
              is_expected.to eq(cost: 60 + 15)
            end
          end
        end

        context "none of them with minimum duration of 0 hours" do
          context "and rounded to hour time range" do
            context "which is below lowest duration rate minimum duration" do
              ...
              it "uses usage rate" do
                create(:duration_rate, instrument: product)

                is_expected.to eq(cost: 120)
              end
            end
        ...
        context "multiple of them" do
          context "and not rounded to hour time range" do
            context "which is above highest duration rate minimum duration" do
              ...
              it "uses both usage rate and duration rates" do
                create(:duration_rate, instrument: product)
                create(:duration_rate, instrument: product, min_duration: 1, rate: 90)
                create(:duration_rate, instrument: product, min_duration: 5, rate: 30)
