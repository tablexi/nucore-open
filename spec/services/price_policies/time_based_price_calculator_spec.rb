...
RSpec.describe PricePolicies::TimeBasedPriceCalculator do
  ...
  let(:product) {
    ...
    prod.schedule_rules << schedule_rules
    prod.save
    prod
  }

  let(:schedule_rules) { [day_schedule, night_schedule, weekend_schedule] }
  let(:day_schedule) { build(:schedule_rule, :weekday) }
  let(:night_schedule) { build(:schedule_rule, :weekday, :evening, discount_percent: 10) }
  let(:weekend_schedule) { build(:schedule_rule, :weekend, :all_day, discount_percent: 25) }
