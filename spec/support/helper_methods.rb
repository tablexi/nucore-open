...
#
# Simulates placing an order for a service and having it marked complete
# [_ordered_by_]
#   The user who is ordering the service
# [_facility_]
#   The facility with which the order is placed
# [_account_]
#   The account under which the order is placed
# [_reviewed_]
#   true if the completed order should also be marked as reviewed, false by default
def place_and_complete_service_order(ordered_by, facility, account = nil, reviewed = false)
  @facility_account = facility.facility_accounts.create(FactoryGirl.attributes_for(:facility_account))
  @service = facility.services.create(FactoryGirl.attributes_for(:service, facility_account_id: @facility_account.id))
  place_product_order(ordered_by, facility, @service, account)
