...
describe CreditAccountTransaction do
  it "should create using valid attributes" do
    ...
    @account_txn  = @order_detail.credit_account_transactions.create({:account_id => @account.id, :facility_id => @facility.id,
                                                                      :is_in_dispute => false,
  ...
  it "should require a negative transaction amount" do
    @account_txn = CreditAccountTransaction.new({:transaction_amount => -1})
    @account_txn.valid?
  ...
  it "should require an order detail" do
    @account_txn = CreditAccountTransaction.new({:order_detail_id => 1})
    @account_txn.valid?
    @account_txn.errors.on(:order_detail_id).should be_nil

    @account_txn = CreditAccountTransaction.new()
    @account_txn.valid?
