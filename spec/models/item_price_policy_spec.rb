...
describe ItemPricePolicy do
  ...
  context "test requiring items" do
    ...
    it "should not create a price policy for a day that a policy already exists for" do
      ipp     = @item.item_price_policies.create(Factory.attributes_for(:item_price_policy, :start_date => Date.today + 7, :price_group_id => @price_group.id))
    ...
    it "should return the date for the current policies" do
      ipp = @item.item_price_policies.create(:unit_cost => 10.75, :unit_subsidy => 0.75, :restrict_purchase => false, :start_date => Date.today - 7.days, :price_group_id => @price_group.id)
      ipp.save(false) #save without validations
      @item.item_price_policies.create(:unit_cost => 10.75, :unit_subsidy => 0.75, :restrict_purchase => false, :start_date => Date.today + 7.days, :price_group_id => @price_group.id)
    ...
    it "should return the date for upcoming policies" do
      @item.item_price_policies.create(:unit_cost => 10.75, :unit_subsidy => 0.75, :restrict_purchase => false, :start_date => Date.today, :price_group_id => @price_group.id)
      @item.item_price_policies.create(:unit_cost => 10.75, :unit_subsidy => 0.75, :restrict_purchase => false, :start_date => Date.today + 7.days, :price_group_id => @price_group.id)
      @item.item_price_policies.create(:unit_cost => 10.75, :unit_subsidy => 0.75, :restrict_purchase => false, :start_date => Date.today + 14.days, :price_group_id => @price_group.id)
