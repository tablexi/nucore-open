...
describe ProductAccessGroup do
  before :each do
    ...
    @instrument = @product = @facility.instruments.create(Factory.attributes_for(:instrument, :facility_account_id => @facility_account.id))
    @restriction_levels = []
    3.times do
      @restriction_levels << Factory.create(:product_access_group, :product_id => @product.id)
    end
